...
/*   Updated: 2022/10/11 16:11:49 by junji            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	communicate_pipe(t_pipe *pipe_tool, t_path_list *path_list)
{
	...
	if (pipe_tool->pid == 0)
	{
		...
		if (pipe_tool->prev_pipe_in == -1)
			open_error();
		dup2(pipe_tool->prev_pipe_in, 0);
		close(pipe_tool->prev_pipe_in);
		dup2(pipe_tool->curr_pipe_out, 1);
		close(pipe_tool->curr_pipe_out);
...
void	pipex(t_pipe *p_tool, t_path_list *path_list)
{
	while (++p_tool->i < (p_tool->argc - 1))
	{
		...
		if (p_tool->prev_pipe_in >= 0)
			close(p_tool->prev_pipe_in);
		p_tool->prev_pipe_in = p_tool->curr_pipe_in;
		close(p_tool->curr_pipe_out);
	}
	close(p_tool->prev_pipe_in);
